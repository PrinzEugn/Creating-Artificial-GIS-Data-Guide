...
Instead of using the default plotting function, which is somewhat limited, it's better to use ggplot(). Unfortunately ggplot() is horrendously weird when it comes to syntax. Generally, you can conceptualize a plot as a series of layers and attributes, which are created and modified using a series of stacked functions. For example, below geom_point() draws the points on an empty ggplot(), with the given data and aesthetic (*aes()* ) mapping. The rest is just the various things parameters of the plot that need to be deleted or made transparent so our final image is just the image, and doesn't come with a bunch of extraneous labels.
